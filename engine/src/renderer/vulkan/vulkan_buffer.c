...
b8 vulkan_buffer_create(
    ...
    if(result!=VK_SUCCESS){
        KERROR("unable to create buffer because the required memory allocation failed. Error: %i",result);
        return false;
    }
    if(bind_on_create){
...
void vulkan_buffer_copy_to(
    ...
    u64 dest_offset,
    u64 size){
        vulkan_command_buffer tmp_command_buffer;
        vulkan_command_buffer_allocate_and_begin_single_use(context,pool,&tmp_command_buffer);
