...
b8 vulkan_renderer_backend_initialize(renderer_backend* backend, const char* application_name) {
    ...
    context.allocator = 0;//default vulkan allocator for now
    ...
    platform_get_required_extension_names(&required_extensions);//platform specific surface
...
//find the right type of GPU memory to allocate for buffers, images, etc.
i32 find_memory_index(u32 type_filter, u32 property_flags) {
...
b8 vulkan_renderer_backend_end_frame(renderer_backend* backend, f32 delta_time) {
    ...
    //make sure previous frame is not using this image
    if (context.images_in_flight[context.image_index] != 0){//debug and reread the code
