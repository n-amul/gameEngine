#include "core/event.h"
 ...
 //who is listening and what function to call when the event happens.
 typedef struct registered_event {
     void* listener;
     PFN_on_event callback;
 } registered_event;
 
 //Each event code (like key press or mouse move) has a list of listeners interested in it.
 ...
 typedef struct event_system_state {
     ...
     event_code_entry registered[MAX_MESSAGE_CODES];//So each code (up to 16384) has its own list of listeners.
 } event_system_state;
...
void event_system_shutdown(void* state) {
 ...
 //Registers a new listener to an event code.
...
b8 event_unregister(u16 code, void* listener, PFN_on_event on_event) {
 ...
 //Checks who’s registered for the event code and calls their callback function
 //If any callback returns true, it stops — assuming the event was handled.
