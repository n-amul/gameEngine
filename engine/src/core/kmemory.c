...
//TODO: replace with custom string lib
#include<string.h>
#include<stdio.h>
...
static const char* memory_tag_strings[MEMORY_TAG_MAX_TAGS] = {
    ...
    "ENTITY_NODE",
    "SCENE      "};
//TODO: revise memory to see states for every frame, referance logger.c structure
...
void* kallocate(u64 size,memory_tag tag){
    if(tag==MEMORY_TAG_UNKNOWN){
        KWARN("kallocate called using MEMORY_TAG_UNKOWN");
    } 
    if (state_ptr) {
...
void kfree(void* block,u64 size,memory_tag tag){
    if(tag==MEMORY_TAG_UNKNOWN){
        KWARN("kfree called using MEMORY_TAG_UNKOWN");
    }
    if(state_ptr){
        state_ptr->stats.total_allocated -= size;
        state_ptr->stats.tagged_allocations[tag] -= size;
